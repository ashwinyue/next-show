...
const browserFetchTimeout = 120 * time.Second

const browserFetchToolDesc = `抓取网页的完整内容（支持动态渲染）。
...
// DefaultBrowserFetchConfig 默认配置.
func DefaultBrowserFetchConfig() *BrowserFetchConfig {
	return &BrowserFetchConfig{
		Timeout:  browserFetchTimeout,
		Headless: true,
	}
}

// BrowserFetchTool 基于 browseruse 的网页抓取工具.
type BrowserFetchTool struct {
	config *BrowserFetchConfig
}

// NewBrowserFetchTool 创建 browser_fetch 工具.
func NewBrowserFetchTool(config *BrowserFetchConfig) *BrowserFetchTool {
	if config == nil {
		config = DefaultBrowserFetchConfig()
	}
	if config.Timeout == 0 {
		config.Timeout = browserFetchTimeout
	}
	return &BrowserFetchTool{config: config}
}

// Info 返回工具信息.
func (t *BrowserFetchTool) Info(ctx context.Context) (*schema.ToolInfo, error) {
	return &schema.ToolInfo{
		Name: ToolWebFetch,
		Desc: browserFetchToolDesc,
...
// InvokableRun 执行网页抓取.
func (t *BrowserFetchTool) InvokableRun(ctx context.Context, arguments string, opts ...tool.Option) (string, error) {
...
func (t *BrowserFetchTool) fetchSingleURL(ctx context.Context, item WebFetchItem) *webFetchItemResult {
...
func (t *BrowserFetchTool) extractContent(ctx context.Context, browserTool *browseruse.Tool, prompt string) (string, error) {
	// 如果有 prompt，使用智能提取
	if prompt != "" && t.config.ExtractChatModel != nil {
		goal := prompt
	...
	// 否则直接获取页面 HTML 并转换为文本
	result, err := browserTool.Execute(&browseruse.Param{
		Action: browseruse.ActionExtractContent,
		Goal:   stringPtr("summarize the page content"),
	})
	if err != nil {
		return "", err
	}
...
// buildOutput 构建输出.
func (t *BrowserFetchTool) buildOutput(results []*webFetchItemResult) string {
...
func (t *BrowserFetchTool) formatError(errMsg string) string {
	var sb strings.Builder
